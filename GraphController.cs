...
public class GraphController : MonoBehaviour
{
    ...
    {
        List<int> pointsToDisplay = GetLastXPoints(points, GetNumberOfPointsInInterval(timeMode));
    ...
    private void DrawPoints(UILineRenderer graph, List<int> points)
    {
        ...
        float step = (float)grid.GridSize.x / (GetNumberOfPointsInInterval(timeMode) - 1);
    ...
    private int GetNumberOfPointsInInterval(TimeInterval time)
    {
        return time switch
        {
            TimeInterval.Week => 7,
